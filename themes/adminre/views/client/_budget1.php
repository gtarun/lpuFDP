		...
		<tbody>
			...
			<tr>
				<td class="border-none">
					...
					<div class="form-wizard text-center">
						<?php foreach($item['country'] as $country){
							$region[$country->name]	=	$country->name;
							}
							foreach($region as $name){?>
								<a href="javascript:void();" class="label text-muted" style=""><?php echo $name;?></a>
							<?php } ?>
					</div>
				</td>
				<?php foreach($item['tier'] as $tier){?>
				<td class="text-center table_cell bgcolor-tealmedium<?php echo $tier->price_zone;?>">
				<h5 class="semibold ">$<?php echo $tier->min_price;?> - $<?php echo $tier->max_price;?><small class="text-white"> / hour</small></h5>
					...
					<span class="checkbox custom-checkbox custom-checkbox-white mt5">
						<input type="checkbox" data-parsley-multiple="sa3" id="customcheckbox_wb<?php echo $tier->id;?>" value="<?php echo $tier->id;?>" name="tier[]" data-parsley-id="3576" <?php echo (in_array($tier->id,$sel))?'checked="checked"':'';?> disabled="disabled" />
