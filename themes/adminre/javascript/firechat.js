...
(function(Firebase) {
  ...
  Firechat.prototype = {
    ...
    _loadUserMetadata: function(onComplete) {
      ...
      this._userRef.transaction(function(current) {
        if (!current || !current.id || !current.name || !current.pic ) {
          return {
            id: self._userId,
            name: self._userName,
            pic:self._userPic
  ...
  /*P:edited*/
  Firechat.prototype.setUser = function(userId, userName,userPic,callback) {
    var self = this;
    ...
    self._firebase.root().child('.info/authenticated').on('value', function(snapshot) {
      if (snapshot.val() === true) {
        ...
        /*P:edited*/
        self._userPic = userPic.toString();
    ...
    query.once('value', function(snapshot) {
      ...
      for (var userNameKey in usernames) {
        var sessions = usernames[userNameKey],
            userName, userId,userPic,usernameClean;
        ...
        for (var sessionId in sessions) {
          ...
          userPic=sessions[sessionId].pic;
